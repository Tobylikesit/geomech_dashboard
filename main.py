...
# This is the bad version of the dashboard for demo only!
st.set_page_config(
...
def load_data():
    ...
    return file
# For demo
def do_bad_things(data):
    for i in data.columns:
        if data[i].dtypes == float and i not in ['depth']:
            data[i]= data[i] + data[i].mean()*(np.random.rand(data[i].shape[0])-0.5)*0.3
            print(i)
    return data

# @st.cache(suppress_st_warning=True)
def get_data(xl, dir_col, res_d, demo):
    ...
    if not demo:
        ...
        my_prog = st.progress(0)
        shale = pd.read_csv('./YOUR_DATA.csv')
        # st.write(grdepthsh)
    col_bys = ['depth','well']
    if dir_col:
        col_bys.append('dir')
    ...
    with pd.ExcelWriter(filename) as writer:  
        ...
        my_prog = None
    # return do_bad_things(pd.DataFrame(grdepthsh)) #demo data synth
...
def poly_reg(x, y, Plot = True):
    ...
    print("Here")
    right.latex(r'''
              [0] x^{n} + [1] x^{n-1} + \cdots + [n] x^{0}\newline =
              \sum_{k=0}^{n} [k]x^{n-k}
              ''')
    right.json(dict(enumerate(coeffs)))
    return(coeffs)
...
try:
    with st.echo(code_location='below'):
        ...
        # Data Reading
        demo = st.sidebar.checkbox("Run Demo with synthetic data", value=False, help='30 % column mean noise was applied to shales data for security reasons.')
        file = load_data()
        if file == None and (not demo):
            # ds = st.sidebar.radio(
            # "Or choose dataset:",
            ...
            df = get_data(file, dir_col, res_d, demo)
        ...
        if not wells:
            ...
            with st.sidebar.form(key='axes_sidebar'):
                ...
                x_axis = st.selectbox(
                'X-axis column:',
                xaxvalues,index=4%len(xaxvalues))
                
                yaxvalues = [x for x in np.array(df.columns.unique()) if x not in ['well','depth',]]
                
                y_axis = st.selectbox(
                'Y-axis column:',
                yaxvalues,index=5%len(yaxvalues))
